...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    // Ensure cursor is hidden on target elements even if they set `cursor-pointer`
    let targetCursorStyleEl: HTMLStyleElement | null = document.querySelector('style[data-target-cursor-style="true"]');
    if (!targetCursorStyleEl) {
      targetCursorStyleEl = document.createElement('style');
      targetCursorStyleEl.setAttribute('data-target-cursor-style', 'true');
      targetCursorStyleEl.textContent = `${targetSelector}, ${targetSelector} * { cursor: none !important; }`;
      document.head.appendChild(targetCursorStyleEl);
    }
    ...
    return () => {
      ...
      spinTl.current?.kill();
      const existingStyle = document.querySelector('style[data-target-cursor-style="true"]');
      if (existingStyle && existingStyle.parentNode) {
        existingStyle.parentNode.removeChild(existingStyle);
      }
      document.body.style.cursor = originalCursor;
    };
  }, [mounted, targetSelector, spinDuration, moveCursor, constants, hideDefaultCursor]);
